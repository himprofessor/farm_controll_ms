<template>
  ...
  <div class="bg-white rounded-lg shadow p-4">
    ...
    <!-- Borrow Report Table -->
    <div v-if="borrowReports.length" class="overflow-x-auto mt-4">
      <table class="min-w-full bg-white">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Return-Back</th>
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="report in borrowReports" :key="report.id">
            ...
            <td class="px-4 py-2 text-sm text-gray-500">{{ report.retunred_date }}</td>
    ...
    <div v-else class="text-center py-4 text-gray-500">No borrow records found.</div>
  </div>
</template>
...
import API from '@/plugin/axios'  // Axios instance with backend base URL
...
const handleBorrowSubmitted = async (data) => {
  try {
    ...
    console.error('Failed to submit borrow:', error)
  }
}
