...
<template>
  <div>
    ...
    <BorrowForm
      ...
      @submitted="handleBorrowSubmitted"
    />
    ...
    >
      <div class="bg-white rounded-lg shadow-lg p-4 w-80">
        ...
        <p class="text-gray-600 mb-2">
          {{ $t('staff.confirmAction') }}
          {{
            deleteType === 'material'
              ? selectedMaterialToDelete.name
              : `${selectedMaterialToDelete.quantity} ${$t('inventory.of')} ${selectedMaterialToDelete.material.name} ${$t('inventory.borrowedBy')} ${selectedMaterialToDelete.staff.name}`
          }}?
        </p>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200">
      <table class="min-w-full text-sm">
        <thead class="bg-green-600">
          <tr>
            <th class="px-6 py-3 text-left font-semibold text-white uppercase tracking-wider">
            ...
            <th class="px-6 py-3 text-center font-semibold text-white uppercase tracking-wider">
              {{ $t('inventory.actions') }}
            </th>
          </tr>
        </thead>
          ...
          <tr v-for="material in visibleItems" :key="material.id" class="hover:bg-gray-50 transition">
            <td class="px-6 py-4">
              ...
              <div class="text-xs text-gray-500">
                {{ $t('inventory.expires') }}: {{ material.expires || $t('staff.na') }}
              </div>
            </td>
            <td class="px-6 py-4">
            ...
            <td class="px-6 py-4">
              ...
              <div class="text-xs text-gray-500">
                {{ $t('inventory.minStock') }}: {{ material.minStock }} {{ material.unit }}
              </div>
            </td>
            <td class="px-6 py-4">
              <span :class="getStatusClass(material.status)">
                {{ $t(`inventory.status${material.status.charAt(0).toUpperCase() + material.status.slice(1)}`) }}
              </span>
            </td>
            <td class="px-6 py-4">
            ...
            <td class="px-6 py-4">
              ...
              <div class="text-xs text-gray-500">
                {{ $t('inventory.lastUpdated') }}: {{ material.lastUpdated }}
              </div>
            </td>
            <td class="px-6 py-4 text-center relative">
              ...
              >
                ...
                <button
                  @click="showDeleteConfirm = true; selectedMaterialToDelete = material; deleteType = 'material'"
    ...
    <div class="mt-8">
      ...
      <div v-if="isLoading" class="mb-4 text-gray-600 text-sm">{{ $t('inventory.loading') }}</div>
      ...
      <div class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="min-w-full text-sm">
          <thead class="bg-gray-400">
            <tr>
              <th class="px-3 py-3 text-left font-semibold text-white uppercase tracking-wider">
              ...
              <th class="px-3 py-3 text-left font-semibold text-white uppercase tracking-wider">
                {{ $t('inventory.borrowedDate') }}
              </th>
              <th class="px-3 py-3 text-left font-semibold text-white uppercase tracking-wider">
                {{ $t('inventory.returnedDate') }}
              </th>
              <th class="px-3 py-3 text-left font-semibold text-white uppercase tracking-wider">
                {{ $t('inventory.materialName') }}
              </th>
              <th class="px-3 py-3 text-left font-semibold text-white uppercase tracking-wider">
                {{ $t('inventory.staffName') }}
              </th>
              <th class="px-3 py-3 text-left font-semibold text-white uppercase tracking-wider">
                {{ $t('inventory.actions') }}
              </th>
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="report in visibleBorrowReports" :key="report.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 text-sm text-gray-500">
                {{ report.returned_date || $t('inventory.notReturned') }}
              </td>
              ...
              <td class="px-4 py-2 text-sm text-right relative">
                <button
                  @click="showMenuId = showMenuId === report.id ? null : report.id"
                  class="w-6 h-6 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 focus:ring-2 focus:ring-blue-500"
                ...
                >
                  ...
                  <div class="border-t border-gray-200"></div>
                  <button
                    @click="showDeleteConfirm = true; selectedMaterialToDelete = report; deleteType = 'borrow'"
                    class="w-full text-left px-2 py-1 text-sm text-red-500 hover:bg-red-50 flex items-center gap-1"
                  >
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                      <path
...
const translatedCategories = computed(() => [
  { value: 'seeds', label: t('inventory.categories.seeds') },
  { value: 'fertilizer', label: t('inventory.categories.fertilizer') },
  { value: 'tools', label: t('inventory.categories.tools') },
  { value: 'feed', label: t('inventory.categories.feed') },
  { value: 'medicine', label: t('inventory.categories.medicine') }
])

const getCategoryLabel = (category) => {
  const foundCategory = translatedCategories.value.find(cat => cat.value === category)
  return foundCategory ? foundCategory.label : t('inventory.itemUnknown')
}
...
const fetchBorrowReports = async () => {
  ...
  try {
    ...
    errorMessage.value = t('inventory.errorFetchBorrowReports')
  } finally {
...
const handleBorrowSubmitted = async (data) => {
  ...
  try {
    ...
    errorMessage.value = error.response?.data?.message || t('inventory.errorBorrowSubmission')
  } finally {
    isLoading.value = false
  }
}
...
const confirmDelete = async () => {
  ...
  try {
    if (deleteType.value === 'material') {
      ...
      await fetchBorrowReports()
      successMessage.value = t('inventory.successBorrowDeleted')
    }
    setTimeout(() => { successMessage.value = '' }, 3000)
  } catch (error) {
    console.error('Delete error:', error)
    errorMessage.value = t('inventory.errorDelete')
  } finally {
    showDeleteConfirm.value = false
    selectedMaterialToDelete.value = null
    deleteType.value = null
...
const returnBorrow = async (report) => {
  if (report.id) {
    try {
      ...
      errorMessage.value = t('inventory.errorBorrowReturn')
    }
  }
}
