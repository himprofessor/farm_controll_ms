    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-lg p-4 w-80">
        ...
        <p class="text-gray-600 mb-2">
          {{ $t('staff.confirmAction') }} {{ deleteType === 'material' ? selectedMaterialToDelete.name : `${selectedMaterialToDelete.quantity} ${$t('inventory.of')} ${selectedMaterialToDelete.material.name} ${$t('inventory.borrowedBy')} ${selectedMaterialToDelete.staff.name}` }}?
        </p>
...
const categories = ['គ្រាប់ពូជ', 'ជី', 'ឧបករណ៍', 'ចំណី', 'ថ្នាំ']
...
const fetchBorrowReports = async () => {
  ...
  try {
    ...
    errorMessage.value = 'បរាជ័យក្នុងការផ្ទុកកំណត់ត្រាខ្ចី។ សូមព្យាយាមម្តងទៀត។'
  } finally {
...
const handleBorrowSubmitted = async (data) => {
  ...
  try {
    ...
    successMessage.value = 'កំណត់ត្រាខ្ចីត្រូវបានបន្ថែម និងស្តុកត្រូវបានធ្វើបច្ចុប្បន្នភាព!'
    setTimeout(() => { successMessage.value = '' }, 3000)
  } catch (error) {
    console.error('Borrow submission error:', error)
    errorMessage.value = error.response?.data?.message || 'បរាជ័យក្នុងការបន្ថែមកំណត់ត្រាខ្ចី។'
  } finally {
    isLoading.value = false
  }
}
...
const confirmDelete = async () => {
  ...
  try {
    if (deleteType.value === 'material') {
      ...
      emit('delete-item', id)
      successMessage.value = 'សម្ភារៈត្រូវបានលុបដោយជោគជ័យ!'
    } else if (deleteType.value === 'borrow') {
      // Delete borrow record only
      await API.delete(`/borrowings/${id}`)
      await fetchBorrowReports()
      successMessage.value = 'កំណត់ត្រាខ្ចីត្រូវបានលុបដោយជោគជ័យ!'
    }
    setTimeout(() => { successMessage.value = '' }, 3000)
  } catch (error) {
    console.error('Delete error:', error)
    errorMessage.value = 'បរាជ័យក្នុងការលុប។ សូមព្យាយាមម្តងទៀត។'
  } finally {
    showDeleteConfirm.value = false
    selectedMaterialToDelete.value = null
    deleteType.value = null
...
const returnBorrow = async (report) => {
  if (report.id) {
    try {
      ...
      await fetchBorrowReports()
      successMessage.value = 'កំណត់ត្រាខ្ចីត្រូវបានសម្គាល់ថាត្រឡប់!'
      setTimeout(() => { successMessage.value = '' }, 3000)
    } catch (error) {
      console.error('Return error:', error)
      errorMessage.value = 'បរាជ័យក្នុងការសម្គាល់កំណត់ត្រាខ្ចីថាត្រឡប់។ សូមព្យាយាមម្តងទៀត។'
    }
  }
}
